...
public class DungeonViewer extends JPanel {
    ...
    private final int tileSize = 16; // bigger for visibility
    private JTextArea combatLog;
    private final PlayerParty party;
    private final MainWindow mainWindow;
    private boolean movementEnabled = true;
    private CombatSystem currentCombat; // store combat reference
    ...
    public DungeonViewer(Dungeon dungeon, JTextArea combatLog, PlayerParty party, MainWindow mainWindow) {
        ...
        // Load monsters
        for (Dungeon.Position pos : dungeon.getMonsterPositions()) {
            Point p = new Point(pos.x(), pos.y());
            // Random monster type for variety
            Monster m = rand.nextBoolean() ? MonsterList.goblin() : MonsterList.orc();
            monsters.put(p, m);
        }
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        int offsetX = (getWidth() - map[0].length * tileSize) / 2;
        int offsetY = (getHeight() - map.length * tileSize) / 2;
